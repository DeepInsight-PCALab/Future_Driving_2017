...
class ResNeXt(nn.Module):
    ...
    """
    def __init__(self, baseWidth, cardinality, layers, slicing):
        """ Constructor
        Args:
        ...
        self.layer_cls = nn.Conv2d(self.inplanes, 1, kernel_size = 1, stride = 1, padding = 0, bias = True)
        self.layer_up  = nn.Conv2d(self.inplanes, slicing + 1, kernel_size = 1, stride = 1, padding = 0, bias = True)
...
def resnext101(baseWidth, cardinality, slicing):
    ...
    """
    model = ResNeXt(baseWidth, cardinality, [3, 4, 23, 3], slicing)
    return model
