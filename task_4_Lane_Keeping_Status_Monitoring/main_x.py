...
def lane_bubble_sort(x_line_list, up = True):
    new_line_list = []
    for line in x_line_list:
        new_line_list.append((line, line[-1]))
    new_line_list = sorted(new_line_list, key = lambda x:x[1])
    res = []
    for line in new_line_list:
        res.append(line[0])
    return res
def sort_ans(ans, up=True):
    new_ans = sorted(ans, key = lambda x:x[1])
    return new_ans


def lane_bubble_sort_bak(x_line_list, up=True):
...
def calc_world2camera(x0, y0, ow, oh, AA, RR, tt):
    u, v = 0, 0
    A = AA  #np.array([[1652.7548306478222, 0., 645.59843662100081], [0., 1651.4166835094670, 476.44495746759020], [0., 0., 1.]])
    R = np.mat(RR)  # np.mat(np.array([[0.0079177846640020241, -0.99986396117327780, -0.014469548468994465],
    ...
    t = np.mat(tt)  # np.mat(np.array([[3.6554495974876362], [223.23405430707408], [-106.12491050729992]]))
    # Rt = np.mat(Rt)
    # M_ = np.mat(np.array([[1.0 * x0], [1.0 * y0], [106.]]))  # 以相机作为世界坐标系的中心
    M_ = np.mat(np.array([[1.0 * x0], [1.0 * y0], [0.]]))  # 以相机下方的地面作为世界坐标系的中心
    fx = A[0][0]  # 1652.7548306478222
    fy = A[1][1]  # 1651.4166835094670
    cx = A[0][2]  # 645.59843662100081
    cy = A[1][2]  # 476.44495746759020
    xx = R * M_ + t
    ...
    if u >= -504 and u < 2922 and v >= -504 and v < 2999:
        return True, u, v
    else:
        return False, u, v

def binary_search(bx, y, x_line_list, lane_num, ow, oh, AA, RR, tt):
    far = bx
    L, R = -1, -1
    for i in range(-5000, 5000):
        flag, c_x, c_y = calc_world2camera(far, i, ow, oh, AA, RR, tt)
        if flag == True:
            L = i
            break
    ...
    while np.abs(R-L) > eps:
        mid = (L + R) / 2.
        flag, c_x, c_y = calc_world2camera(far, int(mid), ow, oh, AA, RR, tt)
        # print('!', mid, c_x, c_y)
        if flag == False:
            print('ERROR!')
            break
...
if __name__ == '__main__':
    ...
    # img_base_path = "../../FC2017/task_4/TSD-LKSM/"
    
    result_path   = "/data3/XJTU2017/task_4/TSD-LKMS-Result-PCALab"
    # result_path = "../../Results/task_4/TSD-LKMS-Result-PCALab"
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    
    info_path = "/data3/XJTU2017/task_4/TSD-LKSM-Info"
    # info_path = "../../FC2017/task_4/TSD-LKSM-Info"
    
    folder_list = os.listdir(img_base_path)
    LANE_DETECT = getLanes()
    number = 0
    for folder_name in folder_list:
        img_folder = img_base_path + folder_name
        print(img_folder)
        img_list = os.listdir(img_folder)
        result_group = img_folder.split('-')[-1]
        result_save = result_path + '/' + 'TSD-LKSM-' + result_group + '-Result.xml'
        
        info_way = info_path + '/' + 'TSD-LKSM-' + result_group + '-Info.xml'
        print(info_way)
        AA, RR, tt = read_info.read_xml(info_way)
        
        _, u, v = calc_world2camera(600., 0., image_width, image_height, AA, RR, tt)
        
        # print(A, R, t)
        fout = open(result_save, 'w')
        fout.write('{}\n'.format("<?xml version=\"1.0\" encoding=\"gbk\"?>"))
        fout.write('{}\n'.format("<opencv_storage>"))
        tmp_ans = []
        print(result_save)
        ...
        for img_name in img_list:
            ...
            save_idx = img_name.split('-')[-1].split('.')[0]
            print(save_idx)
            # print(img_path)
            # continue
            # exit()
            ...
            front_car = 600
            # 二分查找左车道线
            l_c_x, l_c_y, left_lane_world_y = binary_search(front_car, original_y, original_x_line_list, left_lane, ow, oh, AA, RR, tt)
            
            # 二分查找右车道线
            r_c_x, r_c_y, right_lane_world_y = binary_search(front_car, original_y, original_x_line_list, right_lane, ow, oh, AA, RR, tt)
            ...
            # fout.write('<Frame{}>{:g}</Frame{}>\n'.format(save_idx, distance, save_idx))
            tmp_ans.append((distance, save_idx))
            # break
            # number += 1
            # if number == 30:
                # break
        ...
        tmp_ans = sort_ans(tmp__ans)
        for val, idx in tmp_ans:
            fout.write("<Frame{}>{:g}</Frame{}>\n".format(idx, val))

        fout.write("</opencv_storage>\n")
        fout.close()
        # break
    end_time = time.time()
